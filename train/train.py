...
def loss_pi(predicted,target):
    mult = -target*((1e-8+predicted).log())
    loss = torch.sum(mult, 1)
...
def collate(samples):
    graph, mask, indexmask, targetSpectra, pib, Vb = map(list, zip(*samples))
    graph = dgl.batch(graph)
    len_matrix = get_len_matrix(graph.batch_num_nodes())

    #Converting list of targetSpectra as tensors to a 2 dim tensor
    targetSpectra = torch.stack(targetSpectra)
    ...
    pib = torch.cat(pib,0)
    Vb = torch.cat(Vb,0)
    return [graph, len_matrix,mask, indexmask, targetSpectra, ], pib,Vb
...
def train(model, value_model,episode_actor,wandb):
    ...
    class Dataclass(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            spec = exp[0]['targetSpectra']
            pib = exp[2]
            Vb = exp[1]
            return [graph,\
                    torch.FloatTensor(mask).unsqueeze(0),\
                    torch.FloatTensor(indexmask).unsqueeze(0),\
                    torch.FloatTensor(spec).unsqueeze(0),\
                    torch.FloatTensor(pib).unsqueeze(0),\
                    Vb,]
    ...
    while(1):
        if (ray.get(episode_actor.get_size.remote()) >= PRETRAINING_SIZE ): 
            ...
            train_prior(1, model, optimizer, prior_loader,wandb)
...
def train_prior(epochs,model,optimizer,database,wandb):
    loss_prior_arr = []
    loss_value_arr = []
    loss_arr = []
    ...
    for i in range(epochs):
        for samples in database:
            ...
            data,pib,vb = samples
            output = model(data)
            loss1 = loss_pi(output[0], pib.to(device1))
            loss2 = loss_v(output[1], vb.to(device1))
            loss = loss1 + loss2
            loss.backward()
            optimizer.step()
    ...
    print("Training took {} seconds-----{}".format((time.time() - start_time),np.mean(loss_arr)))
