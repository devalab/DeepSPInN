...
class State():
    ...
    def __init__(self,mol_state,r,terminal,parent_action,na,model,valuemodel, Env):
        ...
        #debugprint('Before rollout reward')
        if not self.terminal:
            self.calc_rollout_reward()
        else:
            self.rollout_reward = 0.0

        #debugprint('Getting child actions')
        # Child actions
        self.na = na 
        ...
        #debugprint('Before model')
        self.model = model
        #debugprint('Before predict pi')
        try:
    ...
    def calc_rollout_reward(self):
        ...
        target_mol = Chem.MolFromSmiles(Chem.MolToSmiles(Chem.MolFromSmiles(self.target_mol, sanitize=False), kekuleSmiles=True), sanitize=False)
        current_mol = Chem.MolFromSmiles(Chem.MolToSmiles(Chem.MolFromSmiles(str(self.mol_state), sanitize=False), kekuleSmiles=True), sanitize=False)

        substructmatch = int(target_mol.HasSubstructMatch(current_mol))

       
        # print(substructmatch, self.mol_state, self.target_mol)
        if substructmatch == 0:
            self.rollout_reward = 0 
        else:
            self.rollout_reward = 1
...
class MCTS():
    ...
    def search(self,n_mcts,c,Env,episode_actor):
        ''' Perform the MCTS search from the root '''
        #debugprint('In MCTS search function') 
        ...
        #debugprint('Starting MCTS steps')
        for i in range(n_mcts):
            ...
            while not state.terminal:
                ...
                #print('Taking step', i)
                return_step = mcts_env.step(int(action.index), episode_actor)
                ...
                if hasattr(action,'child_state'):
                    ...
                    state = action.add_child_state(s1,r,t,self.model,self.valuemodel, Env) # expand    
                    break

            # Back-up 
            #debugprint('Rolling back up')
